...
/*
    ...
    Steps :
     ...
     2. Sort Intervals : sort(begin(intervals), end(intervals)) O(nlon)
    ...
    for(int i = n - 1; i >= 0; i--) {  => O(n)
        ...
        int idx = intervals[i][3];
        int j = nextIdx[i];
        for(int k = 1; k <= 5; k++) {  => O(1)
            skip = dp[i + 1][k];  // We need future values so i is reverse loop
            temp = dp[j][k - 1];  // We need past values so normal k loop

            Node take;
            take.score = temp.score + wt;
            take.indexes - temp.indexes;
            take.indexes.add(idx);
            sort(take.indexes from start to end);  take.size will be less than 4 => O(4log4) => O(1)

            and if-else block same

            dp[i][k] = res;
        }
    }

    return dp[0][k].indexes;

    Overall => O(nlogn) or k is genralized then it changes
*/